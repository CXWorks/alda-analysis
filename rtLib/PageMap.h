...
#ifndef UTIL_DATATYPES_PAGEMAP_H_
#define UTIL_DATATYPES_PAGEMAP_H_
...
//                }
            ...
            const std::pair<bool, pointer> test(const_key_reference key) {
                ...
                return map_[mask_key].load()->test(key);
            }
            ...
            reference get(const_key_reference key, alloc &a) {
                ...
                if (std::atomic_load_explicit(&map_[mask_key], std::memory_order_acquire) == nullptr) {
                    pthread_spin_lock(&lock);
                    if (std::atomic_load_explicit(&map_[mask_key], std::memory_order_acquire) == nullptr) {
                        auto next_page = allocate_page(a);
                        std::atomic_store_explicit(&map_[mask_key], next_page, std::memory_order_release);

                    }
                    pthread_spin_unlock(&lock);
//                    map_[mask_key] =
//                            std::unique_ptr<NextPageType, Deleter>(next_page, Deleter(a));
                }

                return map_[mask_key].load(std::memory_order_acquire)->get(key, a);
            }
...
//            };
            ...
            std::array<std::atomic<NextPageType *>, PageSize> map_;
            //}}}
        };
